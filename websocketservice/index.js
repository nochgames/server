...
var WebsocketService = function(gamemechanics) {
  ...
  this.webSocketServer.on('connection', function(ws) {
    ...
    self.updateEventListener(ws, 'message', stub, stub, self.onMessageMap, self.createDoOnMessageStub(ws, stub));
    self.updateEventListener(ws, 'error', stub, stub, self.onErrorMap, self.createDeleteStub(stub));
    self.updateEventListener(ws, 'close', stub, stub, self.onCloseMap, self.createDeleteStub(stub));
  });

  console.log('The WS-server is running on port %s', 8085);
};
...
WebsocketService.prototype = {
  ...
  sendToPlayer: function(message, reciever, event_name) {
    ...
    try {
      ...
      console.log('Unable to send ' + message + ' to player. Player is\n' + reciever + '\n' + e);
    }
  },
  ...
  createDoOnMessageStub: function(socket, stubCurrent) {
    ...
    return function(message) {
      ...
      if ('startGame' in message) {
        ...
        self.updateEventListener(ws, 'message', stub, player, self.onMessageMap, self.createDoOnMessage(player));
        self.updateEventListener(ws, 'error', stub, player, self.onErrorMap, self.createDoOnError(player));
        self.updateEventListener(ws, 'close', stub, player, self.onCloseMap, self.createDoOnClose(player));
      }
